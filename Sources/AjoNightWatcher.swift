...
struct OpencodeRegistryView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Text("All Zen models in one list. Live rate limits are read from OpenCode's own log, which records no reset time; enter it below to resume.").font(.caption).foregroundStyle(.secondary)
            HSplitView {
                List(filtered, selection: $selection) { task in
                    VStack(alignment: .leading, spacing: 5) {
                        HStack {
...
struct CommandCodeTask: Decodable, Identifiable {
    let id: String
    let title: String
    let cwd: String
    let state: String
...
struct CommandCodeSnapshot: Decodable {
    let tasks: [CommandCodeTask]?
    let prompt: String?
    let cli: String?
    let events: [WatchEvent]?
    let error: String?
...
final class CommandCodeStore: ObservableObject {
    @Published var tasks: [CommandCodeTask] = []
    @Published var prompt = ""
    @Published var cli = ""
    @Published var error = ""
    @Published var busy = false
    private let queue = DispatchQueue(label: "ajo-night-watcher.commandcode-backend")
    private var timer: Timer?
    private var seen = Set(UserDefaults.standard.stringArray(forKey: "seenCommandCodeEvents") ?? [])
    var backend: String { Bundle.main.path(forResource: "commandcode", ofType: "py")! }
    init() {
        call(["op": "tick"])
        timer = Timer.scheduledTimer(withTimeInterval: 15, repeats: true) { [weak self] _ in self?.tick() }
        NSWorkspace.shared.notificationCenter.addObserver(forName: NSWorkspace.didWakeNotification, object: nil, queue: .main) { [weak self] _ in self?.tick() }
    ...
    func call(_ request: [String: Any]) {
        ...
        queue.async {
            ...
            do {
                ...
                let data = output.fileHandleForReading.readDataToEndOfFile()
                process.waitUntilExit()
                let snapshot = try JSONDecoder().decode(CommandCodeSnapshot.self, from: data)
                DispatchQueue.main.async {
                    if let tasks = snapshot.tasks { self.tasks = tasks }
                    if let p = snapshot.prompt { self.prompt = p }
                    if let c = snapshot.cli { self.cli = c }
                    ...
                    UserDefaults.standard.set(Array(self.seen.suffix(200)), forKey: "seenCommandCodeEvents")
                }
            } catch {
                DispatchQueue.main.async { self.error = error.localizedDescription; self.busy = false }
            }
...
struct CommandCodeRegistryView: View {
    @ObservedObject var store: CommandCodeStore
    @State var selection: String?
    @State var filter = ""
    @State var watchedOnly = false
    @State var resetDate = Date().addingTimeInterval(3600)
    ...
    @State var draftCLI = ""
    @State var showIdleConfirmation = false
    var selected: CommandCodeTask? { store.tasks.first { $0.id == selection } }
    var filtered: [CommandCodeTask] { store.tasks.filter { (!watchedOnly || $0.armed) && (filter.isEmpty || ([$0.title, $0.cwd, $0.id].joined(separator: " ")).localizedCaseInsensitiveContains(filter)) } }
    func color(_ state: String) -> Color {
        switch state { case "Running": return .blue; case "Waiting": return .orange; case "Rate limited": return .pink; case "Completed": return .green; case "Needs Input": return .red; default: return .secondary }
    }
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                Image(systemName: "chevron.left.forwardslash.chevron.right").font(.title2).foregroundStyle(.secondary)
                VStack(alignment: .leading) {
                    Text("Command Code").font(.title2.bold())
                    Text("Continue your work when usage resets").foregroundStyle(.secondary)
                }
                Spacer()
                if store.busy { ProgressView().controlSize(.small) }
            ...
            Text("Local transcripts only. Command Code keeps no usage data on disk, so resets need manual entry.").font(.caption).foregroundStyle(.secondary)
            HSplitView {
                List(filtered, selection: $selection) { task in
                    VStack(alignment: .leading, spacing: 5) {
                        HStack {
                ...
                VStack(alignment: .leading, spacing: 14) {
                    if let task = selected {
                        ...
                        HStack {
                            Button("Resume Now") { store.call(["op": "resume", "id": task.id]) }.buttonStyle(.borderedProminent).disabled(store.busy || task.state == "Running")
                            Button("Run log") {
                                let p = NSHomeDirectory() + "/Library/Application Support/Ajo Night Watcher/" + task.id + ".commandcode.log"
                                if FileManager.default.fileExists(atPath: p) { NSWorkspace.shared.open(URL(fileURLWithPath: p)) }
                                else { store.error = "No watcher run log yet for this task." }
                            }
                        }
                        Button("Mark idle after stopping in Command Code…") { showIdleConfirmation = true }.font(.caption)
                        Spacer()
                    } else {
                        Spacer()
                        Image(systemName: "sidebar.left").font(.largeTitle).foregroundStyle(.secondary)
                        Text("Select a task to watch").font(.headline)
                        Text("Sessions are discovered from Command Code's local transcripts. No folders are assumed to be apps.").foregroundStyle(.secondary)
                        Spacer()
                    }
                }.padding().frame(minWidth: 310, maxWidth: 390)
            }
            ...
            HStack {
                Text("\(store.tasks.filter { $0.armed }.count) watched · \(store.tasks.filter { $0.state == "Waiting" && $0.armed }.count) waiting · \(store.tasks.filter { $0.state == "Rate limited" }.count) rate limited").font(.caption).foregroundStyle(.secondary)
                Spacer()
                Text("Local only · Command Code session resume").font(.caption).foregroundStyle(.secondary)
            }
        }.padding(20).frame(minWidth: 830, minHeight: 730)
        .onChange(of: selection) { _ in
            editingResetTime = false
        ...
        .alert("Confirm the task has stopped", isPresented: $showIdleConfirmation) {
            Button("Cancel", role: .cancel) {}
            Button("Mark Idle") { if let id = selection { store.call(["op": "idle", "id": id]) } }
        } message: { Text("Use this only after stopping the task in Command Code. An old Running record cannot reliably tell whether another client is still working.") }
        .sheet(isPresented: $settings) {
            VStack(alignment: .leading, spacing: 14) {
                Text("Settings").font(.title2.bold())
                Text("Command Code executable")
                TextField("Absolute path or executable name", text: $draftCLI).textFieldStyle(.roundedBorder)
                Text("Continuation prompt")
                TextEditor(text: $draftPrompt).font(.body).frame(height: 120).border(Color.secondary.opacity(0.3))
                Text("Continuations run headless via `cmd -p --resume`. The CLI is not installed on this Mac yet, so resume stays opted out until you set a path. If a run needs permissions or a reply, review it in Command Code.").font(.callout).foregroundStyle(.secondary)
                HStack { Spacer(); Button("Cancel") { settings = false }; Button("Save") { store.call(["op": "settings", "cli": draftCLI, "prompt": draftPrompt]); settings = false }.buttonStyle(.borderedProminent) }
            }.padding(24).frame(width: 550)
        }
    }
