...
The OpenCode tab reads up to 300 unarchived sessions from `~/.local/share/opencode/opencode.db` over a read-only SQLite connection (session/message/project tables only; it never reads `auth.json`, `account.json`, or credentials). It derives Idle/Waiting/Rate limited/Needs Input/Completed from the latest recorded message per session. Rate limits are additionally detected from a bounded read-only tail of `~/.local/share/opencode/log/opencode.log`: OpenCode retries a limited turn in-process, so a live limit never reaches the database, and the log line is the only local record that names the session. It carries no reset time — OpenCode's own countdown is held in memory and written nowhere on disk — so a detected limit shows **Rate limited** (free Zen models read **Free usage exceeded**) until you enter the reset time. Explicit timestamps already present in a recorded error are still honored. Headless continuation runs `opencode run -s SESSION_ID PROMPT` in the recorded directory; the CLI path defaults to `opencode` in `PATH` and is configurable in Settings. **Future:** the `opencode` CLI is not installed on this Mac yet, so resume is deferred — session monitoring works without it. Install later with `brew install anomalyco/tap/opencode` and smoke-test resume against a disposable session before relying on auto-resume.
...
The **Command Code** tab watches local Command Code transcripts in `~/.commandcode/projects/*/[session-uuid].jsonl`, using the saved title from the matching `[session-uuid].meta.json` and falling back to the first user prompt and then the working directory. Sidecar files (`*.checkpoints.jsonl`, `*.prompts.jsonl`) are skipped. State comes from the **entry structure only** — each message records `meta.source` as `user` (a real prompt), `model` (a reply) or `tool` (a tool result) — so a trailing tool result or a tool call with no result means the turn is unfinished, while ordinary prose that happens to mention rate limits is never mistaken for one. Command Code writes no plan, quota or reset data to disk and the product states the plan is not readable headlessly, so this tab reads **no usage at all**; limits are recognized only from the watcher's own resume run through the documented exit codes (5 rate limited, 10 insufficient credits), and a limit with no recorded reset becomes **Rate limited** until you enter the time. Headless continuation would run `cmd -p --resume SESSION_ID PROMPT`, but **the `cmd` CLI is not installed on this Mac** (the desktop app bundles no runnable executable), so resume is deferred the same way OpenCode's is — monitoring works without it.
...
| Rate limited | A provider rate limit was recorded for the session — OpenCode free Zen usage exceeded, or a Command Code continuation that exited rate-limited. Enter the reset time to resume; nothing is dispatched automatically. |
...
- Command Code resume (`cmd -p --resume`) is implemented but the `cmd` CLI is not installed on this Mac, so it cannot be exercised yet. Monitoring and manual scheduling work without it.
